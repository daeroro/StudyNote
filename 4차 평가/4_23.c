...
{
        printk(KERN_INFO "Finalize Device Driver\n");
        return 0;
}

static ssize_t mydrv_read(struct file *file, char *buf, size_t count, loff_t *f_pos)
{
#if 0
        if((buf == NULL) || (count < 0))
                return -EINVAL;
...
loff_t mydrv_llseek (struct file *file , loff_t ppos, int off)
{
	printk(KERN_INFO "SEEK_END means end");
